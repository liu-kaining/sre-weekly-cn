...
本文解释了在Kubernetes环境中，Java应用Pod频繁因OOM（内存不足）被杀死的一个常见却易被忽视的原因：Java进程使用的内存不仅仅包括堆内存，还包含大量的非堆内存（如Metaspace、线程栈、JIT编译缓存等）。仅监控堆内存并设置容器内存限制，很可能导致容器总内存超限而被kubelet终止。文章提供了诊断和规避此问题的具体方法，例如使用Native Memory Tracking工具。
